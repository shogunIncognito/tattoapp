<template>
  <div class="contacto-container">
    <div class="contacto-content ring-neon">
      <!-- Sección de información de contacto -->
      <div class="info-contacto">
        <h2>Reportar un fallo</h2>
        <div class="contact-info">
          <div class="info-item">
            <i class="fas fa-exclamation-triangle"></i>
            <span>¿Encontraste un problema en la página? </span>
            <span>¡Háznoslo saber!</span>
          </div>
        </div>
        <div class="info-pagina">
          <span>Tu feedback nos ayuda a mejorar.</span>
        </div>
        <!-- Iconos sociales (opcional) -->

        <div class="redes_pagina">
          <h2>Redes</h2>
        </div>

        <div class="card">


          <a class="socialContainer containerOne" href="https://www.instagram.com/leo0_baq/" target="_blank"
            rel="noopener noreferrer">
            <svg viewBox="0 0 16 16" class="socialSvg instagramSvg">
              <path
                d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z">
              </path>
            </svg>
          </a>

          <!-- CORREO -->


          <!--INSTAGRAM-->
          <a class="socialContainer containerThree" href="#">
            <svg viewBox="0 0 48 48" class="socialSvg twitterSvg">
              <path
                d="M 12.5 6 C 8.9280619 6 6 8.9280619 6 12.5 L 6 35.5 C 6 39.071938 8.9280619 42 12.5 42 L 35.5 42 C 39.071938 42 42 39.071938 42 35.5 L 42 12.5 C 42 8.9280619 39.071938 6 35.5 6 L 12.5 6 z M 12.5 9 L 35.5 9 C 37.450062 9 39 10.549938 39 12.5 L 39 35.5 C 39 37.450062 37.450062 39 35.5 39 L 12.5 39 C 10.549938 39 9 37.450062 9 35.5 L 9 12.5 C 9 10.549938 10.549938 9 12.5 9 z M 13.828125 14 L 21.564453 25.056641 L 13.822266 34 L 15.865234 34 L 22.46875 26.351562 L 27.820312 34 L 34.257812 34 L 26.025391 22.234375 L 33.136719 14 L 31.136719 14 L 25.123047 20.943359 L 20.265625 14 L 13.828125 14 z M 16.935547 15.695312 L 19.498047 15.695312 L 31.150391 32.304688 L 28.587891 32.304688 L 16.935547 15.695312 z">
              </path>
            </svg>
          </a>


        </div>
      </div>

      <!-- Formulario de contacto -->
      <div class="formulario-contacto">
        <form @submit.prevent="submitForm">
          <!-- Campo para el nombre -->
          <div class="form-group">
            <input v-model="formData.name" type="text" placeholder="Tu nombre" required />
          </div>

          <!-- Campo para el correo -->
          <div class="form-group">
            <input v-model="formData.email" type="email" placeholder="Tu correo electrónico" required />
          </div>

          <!-- Campo para el mensaje -->
          <div class="form-group">
            <textarea v-model="formData.message" placeholder="Describe el problema que encontraste" required></textarea>
          </div>

          <!-- Botones -->


          <div class="botones">
            <button type="submit" class="enviar-btn" :disabled="loading">
              {{ loading ? "Enviando..." : "Enviar reporte" }}
            </button>

            <div class="botones">
              <router-link to="/" class="enviar-btn">Volver</router-link>
            </div>

          </div>



          <!-- Mensajes de éxito o error -->
          <p v-if="successMessage" class="success-message">{{ successMessage }}</p>
          <p v-if="errorMessage" class="error-message">{{ errorMessage }}</p>

        </form>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      formData: {
        name: "",
        email: "",
        message: "",
      },
      loading: false,
      successMessage: "",
      errorMessage: "",
    };
  },
  methods: {
    async submitForm() {
      this.loading = true;
      this.successMessage = "";
      this.errorMessage = "";

      try {
        const response = await fetch("https://formsubmit.co/ajax/d636d91bf5a448a0bb934bfad607e367", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
          },
          body: JSON.stringify(this.formData),
        });

        const result = await response.json();

        if (result.success) {
          this.successMessage = "¡Gracias! Tu reporte ha sido enviado.";
          this.formData = { name: "", email: "", message: "" }; // Limpiar el formulario
        } else {
          this.errorMessage = "Hubo un error al enviar el reporte. Inténtalo de nuevo.";
        }
      } catch (error) {
        this.errorMessage = "Error de conexión. Por favor, inténtalo más tarde.";
        console.error("Error al enviar el formulario:", error);
      } finally {
        this.loading = false;
      }
    },
  },
};
</script>


<!-- IMPORTE CSS -->

<style>
.success-message,
.error-message {
  color: white;
}



.contacto-container {
  min-height: 100vh;
  display: flex;
  justify-content: center;
  align-items: center;
  padding: 40px;
  background-color: #000;
}

.contacto-content {
  display: flex;
  width: 100%;
  max-width: 1200px;
  background-color: #1a1a1a;
  border-radius: 10px;
  overflow: hidden;
}

.info-contacto {
  flex: 1;
  padding: 40px;
  color: #fff;
  display: flex;
  flex-direction: column;
  align-items: center;
}

.info-contacto h2 {
  color: #00e676;
  margin-bottom: 30px;
  font-size: 28px;
  text-align: center;
}

.contact-info {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 10px;
  margin-bottom: 20px;
  width: 100%;
}

.info-item {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 10px;
  width: 100%;
}

.info-item i {
  color: #00e676;
  width: 20px;
  margin-top: 3px;
  font-size: 18px;
}

.info-pagina {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 10px;
  margin-bottom: auto;
  width: 100%;
}

.card {
  width: fit-content;
  height: fit-content;
  display: flex;
  gap: 20px;
  margin-top: 10px;
}

.socialContainer {
  width: 40px;
  height: 40px;
  background-color: rgb(44, 44, 44);
  display: flex;
  align-items: center;
  justify-content: center;
  border-radius: 50%;
  cursor: pointer;
  transition: all 0.3s ease;
}

.socialContainer:hover {
  transform: translateY(-5px);
}

.socialSvg {
  width: 20px;
  fill: white;
}

.containerOne:hover {
  background: linear-gradient(45deg, #405de6, #5851db, #833ab4, #c13584, #e1306c, #fd1d1d);
}

.containerTwo:hover {
  background: linear-gradient(45deg, #fff 0%, #C5221F 80%);
}

.containerThree:hover {
  background: #0077b5;
}

.formulario-contacto {
  flex: 2;
  padding: 40px;
  background-color: #1a1a1a;
}

.form-group {
  margin-bottom: 20px;
}

input,
textarea {
  width: 100%;
  padding: 12px;
  border: 1px solid #333;
  border-radius: 5px;
  font-size: 16px;
  background-color: #222;
  color: #fff;
}

input::placeholder,
textarea::placeholder {
  color: #888;
}

textarea {
  resize: vertical;
  min-height: 120px;
}

input:focus,
textarea:focus {
  outline: none;
  border-color: #00e676;
}

.botones {
  display: flex;
  gap: 10px;
  justify-content: flex-end;
}

.enviar-btn {
  background-color: #00e676;
  color: #fff;
  border: none;
  padding: 12px 30px;
  border-radius: 5px;
  font-size: 16px;
  cursor: pointer;
  transition: background-color 0.3s ease;
  text-decoration: none;
}

.enviar-btn:hover {
  background-color: #555;
  box-shadow: 0px 0px 10px #33ff5850, 0px 0px 40px #33ff5850, 0px 0px 60px #33ff5850;
}

@media (max-width: 768px) {
  .contacto-content {
    flex-direction: column;
  }

  .info-contacto,
  .formulario-contacto {
    width: 100%;
  }

  .info-item {
    font-size: 14px;
  }

  .info-contacto {
    padding: 30px;
  }

  .formulario-contacto {
    padding: 30px;
  }
}
</style>